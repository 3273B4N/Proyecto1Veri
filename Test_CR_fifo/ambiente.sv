...
class ambiente #(parameter width =16, parameter depth = 8);
  ...
  monitor ##(.width(width)) monitor_inst; //se agrega esto
  ...
  trans_fifo_mbx mon_chkr_mbx;//se agrega esto
  ...
  function new();
    ...
    drv_chkr_mbx   = new();
    mon_chkr_mbx   = new(); //se agrega esto
    agnt_drv_mbx   = new(); 
    chkr_sb_mbx    = new();
    test_sb_mbx    = new();
    test_agent_mbx = new();
    ...
    monitor_inst.mon_chkr_mbx   = mon_chkr_mbx; //se agrega esto
    driver_inst.drv_chkr_mbx    = mon_chkr_mbx; //se agrega esto
