...
rm -rfv `ls |grep -v ".*\.sv\|.*\.sh\|plusargs\.txt"` # ese comando destruye todos los archivos que no sean fuente, se debe descomentar al inicio y se debe comentar cada vez qeu se ejecute este archivo pq sino borra todo, solo se descomenta la primera vez que se usa el tb
